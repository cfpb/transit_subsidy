...
function compute_totals() {
	  //if( !isNaN( num = parseFloat(this.value)) )
    ...
    //Should do at another time
    //$("#frm_smartrip").validate().element( "#id_number_of_workdays" );
    //$("#frm_smartrip").validate().element( "#id_amount" );
}
...
segment_data = [ ]; //
//{u'short_name': u'Metro', u'locality': u'DC Metro', u'distribution_method': u'Smartip', 
//u'url_link': u'http:
...
$("input[name='work_sked']").click(function() {
  ...
  */
  ...
  $('.rm').live('click', function() {
      ...
      //console.log('removing #row_' + counter); 
      $('#row_' + counter).remove();
  ...
  $(".segment_amount").live("keyup", function(event) {
    ...
    /**
     * Computes the cost of the monthly claim based on the number of days the employee
     * commutes times the cost of a daily round-trip fair.
     * */
     /*$('#id_daily_roundtrip_cost').blur( function(){
         rt = $('#id_daily_roundtrip_cost').val();
         days = $('#id_number_of_workdays').val();
         parking = $('#id_daily_parking_cost').val();
         p_tot =   isNaN(parseFloat(parking))? 0 : parseFloat(parking);

         console.log(  p_tot )

         total = (rt * days + p_tot).toFixed(2);
     ...
     * */
     ...
     //Yes, I know this is bad. Stop-gap.
     var valid_segments = function(){
         return true;
         var seg1 = $('#segment-type_1 option:selected');
         var amt1 = $('#segment-amount_1');

         var seg2 = $('#segment-type_2 option:selected');
         var amt2 = $('#segment-amount_2');

         var seg3 = $('#segment-type_3 option:selected');
         var amt3 = $('#segment-amount_3');

         var seg4 = $('#segment-type_4 option:selected');
         var amt4 = $('#segment-amount_4');

         var seg5 = $('#segment-type_2 option:selected');
         var amt5 = $('#segment-amount_2');

         //console.log( amt1 ); 
         if ( seg1.val() == '' ) {
           $('#segment-type_1').focus();
           return false;
         }
         else if ( amt1.val() == '' ){
           amt1.focus();
    ...
    * */
    $('#btn_enroll_smartrip').click( function(){
          if( !valid_segments() ){
            alert( 'Select one or more commuting segments and specify a daily one-way cose' );
            return;
          }
